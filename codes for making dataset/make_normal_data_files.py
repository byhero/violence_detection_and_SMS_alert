...
for video_xml_path, video_file_path , video_file_name in zip(video_xml_paths, video_file_paths, video_file_names):
    ...
    if not os.path.exists('encoding_yet_normal'):
            os.makedirs('encoding_yet_normal')

    _path = 'encoding_yet_normal/'+video_file_name
    os.mkdir(_path)
    ...
    for i in range(min_start - 1):
        ffmpeg_extract_subclip(video_file_path, i, i + 1, targetname=os.path.join(_path , video_file_name +"_"+ str(i) + "_" + 'normal' + ".mp4"))

    # for i in range(max_end + 1 , duration - 1):
    #     ffmpeg_extract_subclip(video_file_path, i, i + 1, targetname=os.path.join(_path , video_file_name +"_"+ str(i) + "_" + 'normal' + ".mp4"))
